...
def GD(x,y, w, alpha, num_iters, lambda_, epsilon):
    ...
    for i in range(num_iters):
        g = 1 / N * np.matmul(np.matmul(Z,np.diagflat(1/(1+np.exp(-1 * np.matmul(w.T, Z)))-np.ones((1, N)))),(np.ones((N,1)))) + 2 * lambda_ * w
        w = w - alpha * g
...
gde = GD(x.T,y,w, alpha, num_iters, lambda_, epsilon)
print(gde)

err = cost(x.T,y,gde,0.1)
print(err)
